export const IT_OPERATIONAL_MESSAGES = {
  "render.title": "Mappatura componente → dashboard",
  "render.desc": "Quando un alert Grafana ha label <code>component=X</code>, la push ntfy risultante riceve un action button \"Open &lt;label&gt;\" verso l'URL. Gli URL che iniziano con <code>/</code> vengono appesi a <code id=\"gbase\">https://grafana.example.com</code>.",
  "render.desc_before_base": "Quando un alert Grafana ha label <code>component=X</code>, la push ntfy risultante riceve un action button \"Open &lt;label&gt;\" verso l'URL. Gli URL che iniziano con <code>/</code> vengono appesi a",
  "render.desc_after_base": ".",
  "render.button_label": "Etichetta pulsante",
  "render.add_row": "+ Aggiungi riga",
  "render.open_title": "Apri URL in un nuovo tab",
  "render.saved_mappings": "Salvate {count} mappature ✓",
  "render.none_freeform": "(nessuno — libero, senza pulsante)",
  "render.runtime_settings": "Impostazioni runtime",
  "render.grafana_base": "URL base Grafana",
  "render.public_url": "URL pubblico klaxond",
  "render.grafana_render_base": "URL base render Grafana",
  "render.grafana_render_token": "Token render Grafana",
  "render.clear_render_token": "Cancella token render Grafana",
  "render.image_ttl": "TTL immagini in secondi",
  "render.ack_ttl": "TTL default ack in secondi",
  "render.env_override": "override da env: {keys}",

  "preview.title": "Anteprima rendering",
  "preview.desc": "Incolla un payload campione (forma Grafana Alertmanager O Beszel), scegli la severità e vedi cosa verrebbe inviato a ntfy. <b>Si aggiorna live</b> mentre scrivi (debounce 500ms). Non viene consegnato nulla.",
  "preview.payload": "Payload (JSON)",
  "preview.load_grafana": "Carica campione Grafana",
  "preview.load_beszel": "Carica campione Beszel",
  "preview.load_healthchecks": "Carica campione Healthchecks",
  "preview.load_wud": "Carica campione WUD",
  "preview.load_shelfmark": "Carica campione Shelfmark",
  "preview.load_prowlarr": "Carica campione Prowlarr",
  "preview.load_decypharr": "Carica campione Decypharr",
  "preview.visual": "Anteprima visiva",
  "preview.mock_empty": "incolla un payload + Renderizza",
  "preview.raw": "Header ntfy raw + body",
  "preview.invalid_json": "JSON non valido: {message}",
  "preview.error_rendering": "Errore nel rendering anteprima",
  "preview.empty_body": "(body vuoto)",

  "routing.title": "Instradamento canali",
  "routing.desc": "La configurazione di routing viene salvata in <code>/data/klaxond.toml</code> o nei sidecar. Le env var di compose hanno comunque precedenza quando impostate.",
  "routing.base_url": "URL base",
  "routing.ntfy_topics": "Topic ntfy",
  "routing.ntfy_topics_desc": "Ogni topic ha un nome (topic id ntfy), un bearer token opzionale e una lista di severità che <i>gestisce</i>. La stessa severità in più topic → fan-out (notifica inviata a tutti i topic che matchano). Le severità possono essere qualunque stringa non vuota (es. <code>info</code>, <code>warning</code>, <code>critical</code>, o custom come <code>low</code>/<code>page</code>).<br>Token <code>***SET***</code> = mantieni esistente; vuoto = cancella (le env var ripopolano al prossimo reload i nomi legacy info/warning/critical).",
  "routing.add_topic": "+ Aggiungi topic",
  "routing.save_topics": "Salva topic",
  "routing.chat_id": "Chat ID",
  "routing.api_base": "URL base API",
  "routing.host": "Host",
  "routing.port": "Porta",
  "routing.from_addr": "Indirizzo mittente",
  "routing.to_addr": "Indirizzo destinatario",
  "routing.bot_token_label": "Token bot",
  "routing.clear_bot_token": "Cancella token bot",
  "routing.user_label": "Username",
  "routing.password_label": "Password",
  "routing.starttls": "Usa STARTTLS",
  "routing.clear_smtp_password": "Cancella password SMTP",
  "routing.save": "Salva routing",
  "routing.url_overridden_env": "url sovrascritto da env",
  "routing.chat_overridden_env": "chat_id sovrascritto da env",
  "routing.api_base_overridden_env": "api_base sovrascritto da env",
  "routing.bot_token_overridden_env": "token bot sovrascritto da env",
  "routing.host_overridden_env": "host sovrascritto da env",
  "routing.user_overridden_env": "utente sovrascritto da env",
  "routing.password_overridden_env": "password sovrascritta da env",
  "routing.bot_token": "bot token:",
  "routing.user": "utente:",
  "routing.password": "password:",
  "routing.saved": "Salvato ✓ (le env var hanno ancora precedenza se impostate)",
  "routing.topic_name": "Nome topic",
  "routing.topic_placeholder": "topic id ntfy",
  "routing.token": "Token",
  "routing.keep_existing_placeholder": "(mantieni esistente — lascia ***SET***)",
  "routing.token_placeholder": "tk_... (o vuoto per fallback env)",
  "routing.token_set": "✓ token impostato",
  "routing.clear_to_remove": "— svuota il campo per rimuovere",
  "routing.no_token": "✗ nessun token impostato",
  "routing.handles": "Severità gestite (separate da virgola, qualunque stringa)",
  "routing.delete_topic": "Elimina topic",
  "routing.summary": "{count} topic · severità instradate: {severities}",
  "routing.none": "nessuna",
  "routing.saved_topics": "Salvato ✓ ({count} topic, severità: {severities})",

  "ingest.title": "Auth webhook in ingresso",
  "ingest.sub": "(0.9.18+, secret condiviso per sorgente)",
  "ingest.desc": "Un secret condiviso abilita e protegge ogni sorgente ingest. L'emitter DEVE presentarlo tramite uno di questi metodi:<br>1. header <code>Authorization: Bearer &lt;secret&gt;</code> (consigliato)<br>2. header <code>X-Klaxond-Token: &lt;secret&gt;</code><br>3. query parameter <code>?token=&lt;secret&gt;</code><br>Le sorgenti senza secret sono <b>disabilitate</b> e rifiutano le consegne. Env <code>KLAXOND_INGEST_SECRET_&lt;SOURCE&gt;</code> sovrascrive TOML.",
  "ingest.secret_source": "Origine secret",
  "ingest.endpoint": "Endpoint",
  "ingest.custom": "Personalizzata",
  "ingest.custom_help": "Usa una sorgente personalizzata per qualsiasi servizio in grado di inviare un webhook JSON.",
  "ingest.add_source": "Aggiungi sorgente",
  "ingest.source_id": "ID sorgente",
  "ingest.source_id_help": "Scegli un ID stabile in minuscolo con lettere, numeri e trattini singoli (ad esempio home-assistant).",
  "ingest.display_name": "Nome visualizzato",
  "ingest.display_name_help": "Scegli il nome leggibile mostrato nel flusso e nelle consegne.",
  "ingest.remove": "Rimuovi",
  "ingest.remove_confirm": "Rimuovere la sorgente personalizzata {source}? Il relativo endpoint webhook e il secret salvato smetteranno di funzionare.",
  "ingest.copy_secret": "Copia ora questo secret nell'emitter {source}. Non verrà mostrato di nuovo. Endpoint: {endpoint}",
  "ingest.note": "\"Genera\" crea lato server un secret hex di 64 caratteri; il valore viene mostrato <b>una sola volta</b> — copialo subito nell'emitter. \"Cancella\" rimuove il secret e disabilita la sorgente.",
  "ingest.secret_set": "✓ secret impostato",
  "ingest.disabled": "disabilitata — nessun secret",
  "ingest.env_readonly": "env (sola lettura, imposta {name})",
  "ingest.generate": "Genera",
  "ingest.set_custom": "Imposta custom…",
  "ingest.clear": "Cancella",
  "ingest.secret_too_short": "Il secret deve contenere almeno 16 caratteri.",
  "ingest.generated": "Secret ingest generato per {source}.",
  "ingest.action_ok": "Ingest-auth {action} OK per {source}.",
  "ingest.added": "Sorgente personalizzata {source} aggiunta.",
  "ingest.removed": "Sorgente personalizzata {source} rimossa.",

  "cascade.title": "Catena tier cascata",
  "cascade.desc": "L'ordine conta: il primo tier viene sempre provato; i tier successivi solo se la cascata è attiva (sempre attiva per <code>/beszel/*</code>, condizionata per <code>/webhook/*</code>). Trascina le righe per riordinare, modifica i timeout. Persistito in <code>/data/klaxond.toml</code>.",
  "cascade.timeout": "Timeout (s)",
  "cascade.timeout_warning_title": "Sicurezza del timeout di consegna",
  "cascade.timeout_warning": "I timeout sono personalizzabili da 1 a 60 secondi. Per ntfy mantieni almeno 15 secondi: un timeout più breve può scadere dopo che ntfy ha accettato il messaggio e attivare una consegna fallback duplicata.",
  "cascade.timeout_risk": "Attenzione: ntfy è impostato a {timeout}s, sotto i {recommended}s consigliati. Può causare notifiche duplicate.",
  "cascade.timeout_risk_title": "Conferma timeout ntfy basso",
  "cascade.timeout_invalid": "Ogni timeout deve essere un numero intero da {min} a {max} secondi.",
  "cascade.low_timeout_confirm": "ntfy è impostato a {timeout}s, sotto i {recommended}s consigliati. Può causare notifiche duplicate. Salvare comunque?",
  "cascade.saved_with_warning": "Salvato con timeout ntfy basso. Sono possibili consegne duplicate.",
  "cascade.add_tier": "+ Aggiungi tier",
  "cascade.default_webhook": "abilitata di default per /webhook/* (Grafana)",
  "cascade.move_up": "Sposta su",
  "cascade.move_down": "Sposta giù",
  "cascade.saved": "Salvati {count} tier ✓",
  "cascade.runtime_toggled": "Cascata runtime aggiornata.",

  "delivery.title": "Consegna — policy + regole",
  "delivery.desc": "Le policy definiscono <em>come</em> klaxond consegna un alert (cascade = fermati al primo OK; broadcast = invia tutti i tier in parallelo). Le regole scelgono una policy in base alle label dell'alert (first-match wins, regex con prefisso <code>re:</code>). Se nessuna regola matcha, si applica la <strong>policy default</strong>. <code>cascade</code> è la policy sintetica costruita dal blocco globale <code>[cascade]</code> in <code>klaxond.toml</code>.",
  "delivery.default_policy_title": "Policy default (fallback no-match)",
  "delivery.default_policy": "Policy default",
  "delivery.policies": "Policy",
  "delivery.tiers_order": "Tier (in ordine)",
  "delivery.add_policy": "+ Aggiungi policy",
  "delivery.rules_title": "Regole (first-match wins)",
  "delivery.match_header": "Match (label = valore, prefisso <code>re:</code> per regex)",
  "delivery.policy": "Policy",
  "delivery.add_rule": "+ Aggiungi regola",
  "delivery.save": "Salva config consegna",
  "delivery.saved": "Salvate {policies} policy, {rules} regole ✓",

  "dedup.title": "Controllo rumore notifiche",
  "dedup.desc1": "Configura ogni sorgente in modo indipendente. Il <b>raggruppamento dei burst</b> attende brevemente e combina gli eventi simultanei prima dell'invio. La <b>soppressione delle ripetizioni</b> invia subito la prima notifica, poi sopprime le notifiche renderizzate identiche durante il cooldown.",
  "dedup.critical_notice": "<b>Le notifiche critiche sono protette per impostazione predefinita:</b> bypassano entrambi i controlli, salvo attivazione esplicita per sorgente o regola selettiva.",
  "dedup.pending_note": "I conteggi dei raggruppamenti in attesa vengono aggiornati all'apertura della pagina.",
  "dedup.mode": "Modalità di gestione",
  "dedup.mode_immediate": "Invia immediatamente",
  "dedup.mode_group": "Raggruppa i burst",
  "dedup.mode_suppress": "Sopprimi le ripetizioni",
  "dedup.mode_group_suppress": "Raggruppa, poi sopprimi le ripetizioni",
  "dedup.grouping_title": "Raggruppamento burst",
  "dedup.repeat_title": "Soppressione ripetizioni",
  "dedup.strategy": "Strategia",
  "dedup.key_recommended": "key (consigliata)",
  "dedup.time": "tempo",
  "dedup.grouping_window": "Finestra di raccolta",
  "dedup.repeat_window": "Cooldown dopo una consegna riuscita",
  "dedup.override_critical": "Raggruppa anche le notifiche critical",
  "dedup.override_title": "Le notifiche critical bypassano il raggruppamento dei burst per impostazione predefinita.",
  "dedup.repeat_override_critical": "Sopprimi anche le ripetizioni critical",
  "dedup.repeat_override_title": "Le notifiche critical bypassano la soppressione delle ripetizioni per impostazione predefinita.",
  "dedup.repeat_identity": "Identità ripetizione: sorgente + severità + titolo, corpo, priorità e tag renderizzati. Gli URL temporanei delle azioni sono ignorati.",
  "dedup.source_defaults": "Impostazioni predefinite per sorgente",
  "dedup.save": "Salva controllo rumore",
  "dedup.rules_title": "Regole selettive",
  "dedup.rules_desc": "Vince la prima regola corrispondente. Sopprimi solo le ripetizioni desiderate oppure crea un'eccezione da inviare sempre; per le altre notifiche valgono le impostazioni della sorgente qui sotto.",
  "dedup.rule_add": "+ Aggiungi regola",
  "dedup.rules_empty_title": "Nessuna regola selettiva",
  "dedup.rules_empty_desc": "Le impostazioni della sorgente si applicano a tutte le notifiche. Aggiungi una regola per usare un cooldown diverso o inviare sempre uno specifico alert.",
  "dedup.rule_enabled": "Attiva",
  "dedup.rule_name_placeholder": "Nome regola",
  "dedup.rule_when": "Quando",
  "dedup.rule_then": "Allora",
  "dedup.rule_field": "Campo",
  "dedup.rule_field_title_or_body": "Titolo o corpo",
  "dedup.rule_field_title": "Titolo",
  "dedup.rule_field_body": "Corpo",
  "dedup.rule_field_alertname": "Nome alert",
  "dedup.rule_field_label": "Label",
  "dedup.rule_label_name": "Nome label",
  "dedup.rule_operator": "Confronto",
  "dedup.rule_operator_contains": "Contiene",
  "dedup.rule_operator_exact": "Uguale a",
  "dedup.rule_operator_regex": "Espressione regolare",
  "dedup.rule_pattern": "Valore o espressione",
  "dedup.rule_pattern_placeholder": "disk|filesystem oppure una frase",
  "dedup.rule_case_sensitive": "Maiuscole/minuscole significative",
  "dedup.rule_action": "Azione",
  "dedup.rule_action_suppress": "Sopprimi ripetizioni identiche",
  "dedup.rule_action_bypass": "Invia sempre",
  "dedup.rule_cooldown": "Cooldown",
  "dedup.rule_include_critical": "Includi notifiche critical",
  "dedup.rule_move_up": "Sposta regola in alto",
  "dedup.rule_move_down": "Sposta regola in basso",
  "dedup.rule_delete": "Elimina regola",
  "dedup.rule_error_name": "Regola {position}: inserisci un nome.",
  "dedup.rule_error_pattern": "Regola {position}: inserisci un valore o un'espressione regolare.",
  "dedup.rule_error_label": "Regola {position}: inserisci il nome della label.",
  "dedup.rules_too_many": "La sorgente {source} può avere al massimo 50 regole.",
  "dedup.rule": "Regola",
  "dedup.source_default": "Default sorgente",
  "dedup.pending": "{count} in attesa",
  "dedup.saved": "Controlli rumore salvati",
  "dedup.duration_seconds": "{count} secondi",
  "dedup.duration_minutes": "{count} minuti",
  "dedup.duration_hours": "{count} ore",
  "dedup.duration_days": "{count} giorni",
  "dedup.recent_suppressed": "Ripetizioni soppresse di recente",
  "dedup.recent_suppressed_desc": "Attività cooldown persistente dal database di history selezionato.",
  "dedup.last_sent": "Ultimo invio",
  "dedup.last_suppressed": "Ultima soppressione",
  "dedup.suppressed_count": "Conteggio",
  "dedup.next_allowed": "Prossimo invio",
  "dedup.available_now": "Ora",
  "dedup.no_recent_suppressed": "Nessuna notifica ripetuta è stata ancora soppressa.",
  "dedup.error_loading": "Errore nel caricamento dei controlli rumore: {message}",
  "dedup.source_generic": "Notifiche in ingresso da questa sorgente.",
  "dedup.source_wud": "Aggiornamenti immagini container WUD — key=image.name → 1 notifica per immagine anche se scatta su N host",
  "dedup.source_grafana": "Alert Grafana (via /webhook/*) — key=commonLabels.alertname",
  "dedup.source_beszel": "Metriche host/container Beszel — key=container_name",
  "dedup.source_healthchecks": "Deadman Healthchecks — key=nome check",
  "dedup.source_uptime-kuma": "Eventi monitor Uptime Kuma — key=ID o nome monitor",
  "dedup.source_authentik": "Eventi identità Authentik — key=action:user (raggruppa burst login)",
  "dedup.source_shelfmark": "Eventi libri Shelfmark — key=event:title (download-complete/failed, request approve/reject)",
  "dedup.source_prowlarr": "Eventi indexer/health Prowlarr — key=eventType:message (Health, ApplicationUpdate, ecc.)",
  "dedup.source_decypharr": "Eventi Decypharr Real-Debrid bridge — key=event:hash (download-start/complete/fail per torrent)",
  "dedup.identity_generic": "Identità raggruppamento: chiave specifica della sorgente.",
  "dedup.identity_wud": "Identità raggruppamento: nome immagine.",
  "dedup.identity_grafana": "Identità raggruppamento: nome alert Grafana.",
  "dedup.identity_beszel": "Identità raggruppamento: nome container.",
  "dedup.identity_healthchecks": "Identità raggruppamento: nome check.",
  "dedup.identity_uptime-kuma": "Identità raggruppamento: ID o nome monitor Uptime Kuma.",
  "dedup.identity_authentik": "Identità raggruppamento: azione e utente.",
  "dedup.identity_shelfmark": "Identità raggruppamento: evento e titolo.",
  "dedup.identity_prowlarr": "Identità raggruppamento: tipo evento e messaggio.",
  "dedup.identity_decypharr": "Identità raggruppamento: evento e hash torrent.",


  "test.title": "Invia alert sintetico",
  "test.desc": "Invia un alert reale attraverso la cascata (ntfy → Telegram → SMTP) con il flag runtime della cascata. Conta in \"Consegne recenti\".",
  "test.component": "Componente (anteprima action button)",
  "test.host_label": "Label host",
  "test.host_placeholder": "(opzionale — aggiunta al titolo)",
  "test.body": "Body",
  "test.fire": "Invia alert di test",
  "test.dry_run": "Dry-run",
  "test.dry_run_desc": "(simula la pipeline, nessuna notifica reale — usa /webhook/<sev>?dry_run=1)",
  "test.note": "Quando selezioni un componente, l'alert viene costruito come payload Grafana con <code>commonLabels.component=&lt;chosen&gt;</code>. La pipeline di rendering prende l'URL dashboard corrispondente e aggiunge l'action button, esattamente come un alert reale. Usalo per verificare la config render end-to-end senza aspettare un trigger vero.",
  "test.manual_body": "Smoke test manuale dalla UI admin.",
  "test.synthetic_title": "test sintetico klaxond",
  "test.dry_run_toast": "Dry-run: {verdict} ({reason})",
  "test.would_deliver": "verrebbe consegnato",
  "test.would_suppress": "verrebbe soppresso",
  "test.send_failed": "Invio test fallito: {message}",

  "shortcut.unsaved": "Modifiche non salvate",
  "shortcut.discard_confirm": "Il tab \"{from}\" ha modifiche non salvate. Scartarle e passare a \"{to}\"?",
  "shortcut.discard": "Scarta modifiche",
  "shortcut.title": "Scorciatoie tastiera",
  "shortcut.save": "Salva il tab attivo (clicca il pulsante Salva primario)",
  "shortcut.esc": "Toglie focus dall'input; svuota i filtri di ricerca",
  "shortcut.help": "Mostra questo overlay",
  "shortcut.jump": "Vai al tab per posizione (quando nessun input è a focus)",
  "shortcut.close": "Premi <code>?</code> o clicca fuori per chiudere.",

  "setup.required_title": "Richiesto per la produzione",
  "setup.required_desc": "Completa questi sei controlli prima di esporre Klaxond agli utenti o a Internet.",
  "setup.recommended_title": "Hardening consigliato",
  "setup.recommended_desc": "Controlli opzionali che migliorano resilienza e sicurezza degli account.",
  "setup.recommended_badge": "Consigliato",
  "setup.status.ok": "Completato",
  "setup.status.warn": "Da completare",
  "setup.status.error": "Bloccato",
  "setup.status.partial": "Parziale",
  "setup.status.info": "Opzionale",
  "status.title": "Stato del sistema",
  "status.desc": "Salute dei canali e attività operativa in un colpo d'occhio.",
  "channel.state_not_configured": "non configurato",
  "channel.review_settings": "Verifica impostazioni canale →",
  "deliveries.empty_title": "Nessun alert è ancora passato da Klaxond",
  "deliveries.empty_desc": "Esegui un test dry-run per validare l'instradamento senza contattare un provider, oppure completa prima la checklist di produzione.",
  "deliveries.empty_test": "Esegui un test dry-run",
  "deliveries.empty_setup": "Verifica setup produzione",

  "status.saving": "Salvataggio…",
  "status.uploading": "Upload…",
  "status.testing": "Test…",
  "status.saved": "Salvato ✓",
  "status.restored": "✓ Ripristinati {bytes} byte. Backup pre-ripristino: {backup}",
  "status.none": "(nessuno)",
  "config.apply_import": "Applica import",
  "config.previewing": "Creo anteprima import…",
  "config.preview_ready": "Anteprima import pronta",
  "config.import_preview_title": "Anteprima import: {name}",
  "config.import_kind": "Tipo",
  "config.import_changed": "File modificati",
  "config.import_unchanged": "File invariati",
  "config.import_restore": "Ripristinerà",
  "config.restore_confirm": "Ripristinare la config klaxond da \"{name}\" ({size} byte)?\n\nLa config corrente verrà prima salvata in backup automatico. Sono accettati TOML e bundle JSON completi. Dopo l'upload, lo stato in memoria viene ricaricato. Continuare?",
  "config.restore_title": "Ripristina configurazione",
  "config.restore": "Ripristina",
  "config.restored_toast": "Config ripristinata — ricarica la pagina per aggiornare tutto lo stato UI",
};

export function buildItalianOperationalMessages() {
  return { ...IT_OPERATIONAL_MESSAGES };
}
