...
pub(in crate::handlers) fn passkey_login_start(
    ...
    let cfg = state.cfg();
    let matching = matching_passkeys(&cfg, &user_hint);
...
fn matching_passkeys(cfg: &RuntimeConfig, user_hint: &str) -> Vec<PasskeyRecord> {
    cfg.auth
        .passkeys
        .iter()
        .filter(|record| {
            ...
            .iter()
            .any(|value| value.to_ascii_lowercase() == user_hint)
        })
        .cloned()
        .collect()
}
...
fn finish_login_under_lock(
    ...
    state
        .with_config_write_lock(|| {
            ...
            let mut user = passkey_user(record);
            ...
            passkey_login_response(state, &mut user)
        })
...
fn passkey_login_response(state: &AppState, user: &mut User) -> Response<Body> {
